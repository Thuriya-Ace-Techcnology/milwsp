package org.ace.insurance.productprocess.service;
...
import org.ace.insurance.resourceQuestion.ResourceQuestionFactory;
...
public class ProductProcessService extends BaseService implements IProductProcessService {
	...
	private ISurveyQuestionDAO surveyQestionDAO;
	/**
	 ...
	 */
	@Transactional(propagation = Propagation.REQUIRED)
	public List<ProductProcessDTO> findOldConfPP(String productId, String processId) {
		List<ProductProcessDTO> result = null;
		try {
			result = productProcessDAO.findOldConfPP(productId, processId);
			for(ProductProcessDTO dto : result) {
				SurveyQuestion resDTO = surveyQestionDAO.findSurveyQuestionById(dto.getSurveyQuestionId());
				if(resDTO.getResourceQuestionList() != null) {
					dto.setResourceQuestionDTOs(ResourceQuestionFactory.convertDTOList(resDTO.getResourceQuestionList()));
				}
			}
		} catch (DAOException e) {
