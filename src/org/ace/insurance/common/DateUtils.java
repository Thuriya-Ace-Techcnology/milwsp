...
public class DateUtils {
	...
	public static Date createEndDate(Date inceptionDate, int year, int month, int day) {
		Date result;
		inceptionDate = DateUtils.resetStartDate(inceptionDate);
		result = DateUtils.addYears(inceptionDate, year);
		result = DateUtils.addMonth(result, month);
		// +1 because both inception date and expired date are counted
		result = DateUtils.addDay(result, day + 1);
		return result;
	}
