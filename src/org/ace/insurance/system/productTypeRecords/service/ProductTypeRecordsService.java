...
public class ProductTypeRecordsService extends BaseService implements IProductTypeRecordsService {
	...
	public List<OnlineProductRecordListDTO> collectAllProduct(List<ThirdPartyPremiumRecordsDTO> thirdPartyDTOList,
		...
		List<OnlineProductRecordListDTO> recordList = new ArrayList<>();
		if (thirdPartyDTOList != null) {
			for (ThirdPartyPremiumRecordsDTO thirdPartyDTO : thirdPartyDTOList) {
				...
				records.setProductType("TPL Online");
				recordList.add(records);
			}
		}
		if (travelDTOList != null) {
			for (MTP001 travelDTO : travelDTOList) {
				OnlineProductRecordListDTO records = new OnlineProductRecordListDTO();
				...
				records.setReceipt_no(travelDTO.getTransactionId());
				recordList.add(records);
			}
		}
		if (personalAccidenDTOList != null) {
			for (MPAP001 personalAccidentDTO : personalAccidenDTOList) {
				OnlineProductRecordListDTO records = new OnlineProductRecordListDTO();
				...
				records.setPremium_amount(personalAccidentDTO.getInsuredPersonList().get(0).getPremium() + "");
				records.setProductType("PA");
		...
		if (dtoList != null) {
			for (SpecialForeignTravelDTO dto : dtoList) {
				OnlineProductRecordListDTO records = new OnlineProductRecordListDTO();
				LocalDate paymentDate = Instant.ofEpochMilli(dto.getPaymentDate()).atZone(ZoneId.systemDefault())
						.toLocalDate();
				LocalDate dateOfBirth = Instant.ofEpochMilli(dto.getDateOfBirth()).atZone(ZoneId.systemDefault())
						.toLocalDate();
				records.setBuy_date(paymentDate.getDayOfMonth() + " "
				...
				records.setDateOfBirth(dateOfBirth.getDayOfMonth() + " "
						...
						+ dateOfBirth.getYear());
				records.setPoloicyNo(dto.getPolicyNo());
				recordList.add(records);
			}
		}

		if (thirdPartyDriverList != null) {
			TPDInfoDTO driverInfoDto;	
			for (TPDProposalDTO dto : thirdPartyDriverList) {
				OnlineProductRecordListDTO records = new OnlineProductRecordListDTO();
				records.setBuy_date(dto.getPaymentDate());
				records.setOrderId(dto.getOrderId());
				records.setPaymentStatus(dto.getProposalStatus().getLabel());
				records.setPremium_amount(String.valueOf(dto.getDriverInfoList().get(0).getPremium()));
				records.setProductType("THIRDPARTYDRIVER");
				records.setTransactionFees(0);
				if(dto.getDriverInfoList() != null) {
					driverInfoDto = dto.getDriverInfoList().get(0);
					records.setPeriod_from(driverInfoDto.getStartDate());
					records.setPeriod_to(driverInfoDto.getEndDate());
				}
				recordList.add(records);
