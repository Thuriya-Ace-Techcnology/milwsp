...
@NamedQueries(value = { @NamedQuery(name = "Payment.findAll", query = "SELECT m FROM Payment m "),
		...
		@NamedQuery(name = "Payment.findCancellationPaymentByReferenceNo", query = "SELECT m FROM Payment m WHERE m.referenceNo = :referenceNo and m.complete=false")})
...
public class Payment implements Serializable, IDataModel {
	...
	public Payment(PaymentDTO tempPayment) {
		...
		this.agentCommission = tempPayment.getAgentCommission();
		this.moefCommission = tempPayment.getMoeaCommissoion();
	}

	public Payment(List<Payment> paymentList) {
		if (paymentList != null && !paymentList.isEmpty()) {
			Payment tempPayment = paymentList.get(0);
			this.policyNo =tempPayment.getPolicyNo();
			this.receiptNo = tempPayment.getReceiptNo();
			this.invoiceNo = tempPayment.getInvoiceNo();
			this.bankAccountNo = tempPayment.getBankAccountNo();
			this.referenceNo = tempPayment.getReferenceNo();
			...
			this.medicalFees = tempPayment.getMedicalFees();
			this.agentCommission = tempPayment.getAgentCommission();
			this.moefCommission = tempPayment.getMoefCommission();
		}
	}
	...
	public double getTotalAmount() {
		return getNetPremium() + servicesCharges + administrationFees + stampFees-(agentCommission+moefCommission);
	}
	
	public double getTotalAmountWithoutAgentCommission() {
	...
	public double getMoefCommission() {
		return moefCommission;
	}

	public void setMoefCommission(double moefCommission) {
		this.moefCommission = moefCommission;
	}
