...
import org.ace.insurance.life.enums.CancellationReason;
...
@NamedQueries(value = { @NamedQuery(name = "LifePolicy.findAll", query = "SELECT l FROM LifePolicy l "),
		...
		@NamedQuery(name = "LifePolicy.findByPolicyListNo", query = "SELECT l FROM LifePolicy l WHERE l.policyNo= :lifePolicyNo"),
		...
		@NamedQuery(name = "LifePolicy.findAllPolicyByProposalId", query = "SELECT l FROM LifePolicy l WHERE l.lifeProposal.id = :lifeProposalId order by l.policyNo asc")})
...
public class LifePolicy implements IPolicy, Serializable, ISorter {
	...
	public int getPeriodOfMonths() {
		if (KeyFactorChecker.isPersonalAccident(policyInsuredPersonList.get(0).getProduct())
				|| (KeyFactorChecker.isShoreJobLife(policyInsuredPersonList.get(0).getProduct().getId()) && periodMonth == 6)) {
			return periodMonth;
		} else {
	...
	public String getTimeSlotListStrForGovernment() {
		StringBuffer buffer = new StringBuffer();
		Calendar cal = Calendar.getInstance();
		cal.setTime(activedPolicyStartDate);
			buffer.append(Utils.formattedDate(cal.getTime(), "dd"));
