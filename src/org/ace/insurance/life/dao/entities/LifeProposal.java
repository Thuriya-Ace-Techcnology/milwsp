...
import org.ace.insurance.life.interfaces.IDataModel;
import org.ace.insurance.life.interfaces.IProposal;
...
public class LifeProposal implements Serializable, IDataModel, IProposal {
	...
	@Column (name="IS_AGENT_COMMISSION")
	...
	private String doeNo;
	
	@Transient
	private String policyBusinessYear;	
	@Transient
	...
	public LifeProposal(LifePolicy lifePolicy) {
		...
		this.paymentChannel = lifePolicy.getPaymentChannel();
	...
	public int getPeriodOfInsurance() {
		if (KeyFactorChecker.isPersonalAccident(proposalInsuredPersonList.get(0).getProduct())
				|| (KeyFactorChecker.isShoreJobLife(proposalInsuredPersonList.get(0).getProduct().getId()) && this.periodMonth == 6)) {
			return periodMonth;
		} else {
	...
	public String getDoeNo() {
		return doeNo;
	}

	public void setDoeNo(String doeNo) {
		this.doeNo = doeNo;
	}
	...
	public int hashCode() {
		...
		result = prime * result + ((doeNo == null) ? 0 : doeNo.hashCode());
	...
	public boolean equals(Object obj) {
		...
		if (doeNo == null) {
			if (other.doeNo != null)
				return false;
		} else if (!doeNo.equals(other.doeNo))
			return false;
	...
	public double getTotalPremiumRate(){
		double totalPremiumRate = 0.0;
		for (ProposalInsuredPerson p : getProposalInsuredPersonList()) {
			if (p.getPremiumRate() > 0) {
				totalPremiumRate = totalPremiumRate + p.getPremiumRate();
			}
		}
		return totalPremiumRate;
	}	

	public String getPolicyBusinessYear() {
		return policyBusinessYear;
	}

	public void setPolicyBusinessYear(String policyBusinessYear) {
		this.policyBusinessYear = policyBusinessYear;
	}
	...
	public boolean isAgentCommission() {
		return isAgentCommission;
	}
